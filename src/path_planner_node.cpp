...
public:
    ...
    bool isValid(const ob::State *state) const override
    {
                                              ...
                                              end = octree_->end_leafs_bbx(); it != end; ++it)
        {
            ...
            {
                 ROS_DEBUG("[ValidityChecker] Collision at state (%.2f, %.2f, %.2f)", x, y, z);
                return false;
            }
        }
         ROS_DEBUG("[ValidityChecker] State (%.2f, %.2f, %.2f) is valid.", x, y, z);
        return true;
    }
...
private:
    ...
    void targetCallback(const geometry_msgs::PointStamped::ConstPtr& msg) {
        ...
        if (has_active_target_) {
            ...
            if (distance_sq <= threshold_sq) {
                ...
                latest_target_point_.header.stamp = new_target_stamp;
                return;
            }
                  ...
                  bool simplify)
    {
         ...
         if (solved == ob::PlannerStatus::EXACT_SOLUTION || solved == ob::PlannerStatus::APPROXIMATE_SOLUTION)
         {
             ...
             if (simplify && geo_path.getStateCount() > 2)
             {
                 ...
                 try {
                     double simplification_time = planning_time_limit_ * 0.1;
             ...
             for (size_t i = 0; i < geo_path.getStateCount(); ++i)
             {
                 ...
                 pose_stamped.pose.position.z = current_planning_altitude_;
                 tf2::Quaternion q_out;
                 q_out.setRPY(0, 0, state->getYaw());
             ...
             ROS_WARN("OMPL failed (Status: %s)", solved.asString().c_str());
             return false;
         }
    }
    ...
    void publishVisualization(const geometry_msgs::Point& target, const nav_msgs::Path& path)
    {
        ...
        target_marker.pose.position.z = current_planning_altitude_;
        ...
        target_marker.lifetime = ros::Duration(0); // Постоянный (до DELETEALL)
        ...
        if (!path.poses.empty()) {
            ...
            path_marker.lifetime = ros::Duration(0); // Постоянный (до DELETEALL)
    ...
    // Очищает только маркер пути, оставляя маркер цели
    void clearPathVisualization() {
        if (visualization_pub_.getNumSubscribers() == 0) return;
        visualization_msgs::MarkerArray marker_array;
        visualization_msgs::Marker clear_path_marker;
        clear_path_marker.header.frame_id = map_frame_id_;
        clear_path_marker.header.stamp = ros::Time::now();
        clear_path_marker.ns = "path_planner";
        clear_path_marker.id = 2; // ID маркера пути
        clear_path_marker.action = visualization_msgs::Marker::DELETE;
        marker_array.markers.push_back(clear_path_marker);
        visualization_pub_.publish(marker_array);
    }
    ...
    double current_planning_altitude_;
